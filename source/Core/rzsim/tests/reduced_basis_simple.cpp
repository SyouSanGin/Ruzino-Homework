...
void print_eigenmode_info(const ReducedOrderedBasis& rob, const std::string& name)
{
    std::cout << "\n=== " << name << " ===\n";
    std::cout << "Number of modes: " << rob.basis.size() << std::endl;
    std::cout << "Laplacian size: " << rob.laplacian_matrix_.rows()
...
void test_tetrahedral_mesh()
{
    ...
    for (int i = 0; i < std::min(10, static_cast<int>(rob_custom.eigenvalues.size())); i++) {
        ...
        std::cout << "Mode " << i << ": custom=" << std::setprecision(6) << custom_val
                  << " libigl=" << libigl_val
                  << " diff=" << diff << std::endl;
    }
}
