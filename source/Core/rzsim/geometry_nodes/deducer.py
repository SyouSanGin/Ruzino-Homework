...
def initialize_basis_set():
    """
    Initialize and load the basis set model
    """
    global _basis_set, _device
    
    if _basis_set is not None:
        return "Basis set already initialized"
    
    # Checkpoint path
    checkpoint_path = r"C:\Users\Pengfei\WorkSpace\ShapeSpaceSpectra-reproduction\checkpoints\mesh_lr_dirichlet_20260113_221200"
    ...
    try:
        ...
        return f"Initialized basis set: {task_name}, {_basis_set.num_fields} eigenfunctions, device={_device}"
...
def run_inference_on_vertices(vertices_cuda, eigenfunction_idx=0, shape_code=None):
    ...
    # Log statistics
    print(f"Inference completed: {num_vertices} vertices, eigenfunction {eigenfunction_idx}")
...
def process_vertices_cuda(vertices_cuda):
    ...
    lines.append(f"Bounding box: min=({min_vals[0]:.4f}, {min_vals[1]:.4f}, {min_vals[2]:.4f})")
    lines.append(f"              max=({max_vals[0]:.4f}, {max_vals[1]:.4f}, {max_vals[2]:.4f})")
    lines.append(f"Center: ({center[0]:.4f}, {center[1]:.4f}, {center[2]:.4f})")
    lines.append(f"First {num_to_print} vertices:")
    for i in range(num_to_print):
        lines.append(f"  [{i}] ({vertices_cpu[i][0]:.4f}, {vertices_cpu[i][1]:.4f}, {vertices_cpu[i][2]:.4f})")
