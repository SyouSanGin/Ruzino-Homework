       ...
       private:
            ...
            const std::vector<pxr::GfVec2d>& world_vertices) const
        {
            return [world_vertices](auto... u_values) -> std::vector<double> {
                std::vector<double> u_vec = { static_cast<double>(
                    u_values)... };
                ...
                if (u_vec.size() == 1 && world_vertices.size() >= 2) {
                    double u1 = u_vec[0];
                    double u2 = 1.0 - u1;
                    result[0] = u1 * world_vertices[0][0] +
                                u2 * world_vertices[1][0];  // x
                    result[1] = u1 * world_vertices[0][1] +
                                u2 * world_vertices[1][1];  // y
                }
                ...
                // coordinates u1, u2 The third coordinate is u3 = 1 - u1 - u2
                    ...
                    double u3 = 1.0 - u1 - u2;
                    result[0] = u1 * world_vertices[0][0] +
                                u2 * world_vertices[1][0] +
                                u3 * world_vertices[2][0];  // x
                    result[1] = u1 * world_vertices[0][1] +
                                u2 * world_vertices[1][1] +
                                u3 * world_vertices[2][1];  // y
                }
                else {
                    // Fallback for other cases
                    for (size_t i = 0;
                         i < world_vertices.size() && i < u_vec.size();
                         ++i) {
            ...
            const std::vector<pxr::GfVec3d>& world_vertices) const
        {
            return [world_vertices](auto... u_values) -> std::vector<double> {
                ...
                if (u_vec.size() == 1 && world_vertices.size() >= 2) {
                    ...
                    result[1] = u1 * world_vertices[0][1] +
                                u2 * world_vertices[1][1];  // y
                    result[2] = u1 * world_vertices[0][2] +
                                u2 * world_vertices[1][2];  // z
                }
